...
این مخزن Android/JVM است و سه ماژول اصلی `:calendar-core`، `:calendar` و `:app` دارد. `:calendar-core` یک کتابخانه خالص Kotlin/JVM است؛ UI فقط Android/Compose است. برای iOS، Desktop یا Web Target فعالی وجود ندارد.
...
./gradlew --version
./gradlew :calendar-core:check testDebugUnitTest lintDebug assembleDebug
```
...
```powershell
.\gradlew.bat --version
.\gradlew.bat :calendar-core:check testDebugUnitTest lintDebug assembleDebug
```
...
بدون Android SDK می‌توان مرز و منطق Core را بررسی کرد:
...
# همه بررسی‌های اصلی CI
./gradlew :calendar-core:check testDebugUnitTest lintDebug assembleDebug assembleRelease

# بررسی metadata انتشار از یک مصرف‌کننده مستقل
...
The reusable `:calendar-core` and `:calendar` modules are configured for signed Maven Central
publication. The AGPL-3.0 license remains unchanged. See
[`PUBLISHING.md`](PUBLISHING.md) for Central Portal registration, GPG signing,
GitHub Secrets, local publication verification and the tagged release process.
